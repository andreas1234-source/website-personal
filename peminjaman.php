...
<html lang="en">
    ...
    <body class="sb-nav-fixed">
        <nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
            <!-- Navbar Brand-->
            <a class="navbar-brand ps-3" href="index.php">Bandara Ban</a>
            <!-- Sidebar Toggle-->
        ...
        <tr>
            <td>
                <div class="card bg-info text-white p-3">
                    <h3>Total Data: <?php echo $count1; ?></h3>
                </div>
            </td>
            <td>
                <div class="card bg-danger text-white p-3">
                    <h3>Total Dipinjam: <?php echo $count2; ?></h3>
                </div>
            </td>
            <td>
                <div class="card bg-success text-white p-3">
                    <h3>Total Kembali: <?php echo $count3; ?></h3>
                </div>
            </td>
        </tr>
    </table>
...
if (isset($_POST['filter_tgl'])) {
    ...
    // Memastikan bahwa tanggal mulai dan selesai tidak kosong
    if ($mulai != null && $selesai != null) {
        // Menggunakan prepared statement untuk mencegah SQL injection
        $stmt = $conn->prepare("SELECT * FROM peminjaman p, stock s WHERE s.idbarang = p.idbarang and tanggal BETWEEN ? AND ?");
        $stmt->bind_param("ss", $mulai, $selesai); // Mengikat parameter
        $stmt->execute();
